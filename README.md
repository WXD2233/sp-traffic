...
- `sp` 中文终端看板：下载/上传速率、60 秒趋势、本次流量、上次记录、运行时长和有效并发
...
- 内置工作进程监督器：异常退出后自动退避重启，并保留本次流量、开始时间和暂停状态
...
- 下载内容不落盘；仅保存很小的配置、PID、实时采样和本次/上次字节统计
...
- “本次运行”仅在正常停止后再次启动时清零；工作进程崩溃并自动恢复时不会清零
- “上次记录”在停止或重启服务前自动保存本次下载、上传和运行时长
- 服务已经停止时，“本次运行”显示为 0，“上次记录”保留刚结束的运行数据
- `sudo sp clear` 会同时清除本次统计和上次记录
...
- 本次与上次计数器均为固定数量的单行数字文件，大小只随数字位数缓慢变化
- 工作进程标准输出和错误输出均直接丢弃，不创建追加日志；最近一次异常只覆盖写入一个不足 256 字节的状态记录
- 可用磁盘低于 200 MiB 时会终止活动传输并自动暂停

因此 `/var/lib/sp-traffic` 的项目数据通常只有几 KiB；高并发运行时临时进度文件合计一般也只有几 MiB，并且不会持续累积。systemd 服务也配置为不把工作进程输出写入 journald。

## 崩溃恢复

- 独立监督器持续观察下载工作进程；异常退出后按 2、4、8、16、30 秒退避，最长等待 30 秒，避免崩溃循环持续消耗 CPU
- 每次恢复沿用原来的 `session_started` 和各槽位持久计数，因此“本次运行”不会因崩溃重新计时或归零
- 崩溃前为暂停状态时，恢复后仍保持暂停；正常执行 `sp stop` 后则持久记录为已停止，不会被误判为崩溃会话
- 每轮工作进程使用独立进程组；发生崩溃时会清理残留的下载和槽位子进程，避免孤儿进程重复占用带宽
- systemd 仍保留服务级 `Restart=always` 作为第二层保护；即使监督器本身崩溃，重启后也会根据运行状态继续原会话
- `sp status` 和终端看板会显示连续恢复次数、最近退出码和当前退避时间
